...
// TODO(lucasw) chan has to be 3
std::vector<Uint8> ViewImage::makeTestPixels(const size_t msg_width, const size_t msg_height,
    const size_t chan, size_t& pitch, uint8_t r, uint8_t g, uint8_t b)
{
  pitch = msg_width * chan;
  std::vector<Uint8> pixels;
  pixels.resize(pitch * msg_height);
  for (size_t i = 0; i < pixels.size() / chan; i += 3) {
    pixels[i + 0] = r;
    pixels[i + 1] = g;
    pixels[i + 2] = b;
#if 0
...
void ViewImage::setSurface(const sensor_msgs::ImageConstPtr& msg) {
  ...
  // TODO(lucasw) this is adding latency
  ...
  ROS_INFO_STREAM("surface " << (t1 - t0).toSec());
  return;
}

void ViewImage::setTexture(const sensor_msgs::ImageConstPtr& msg) {
...
void ViewImage::textureFromImageMsg(const sensor_msgs::ImageConstPtr& msg, SDL_Texture* texture) {
  Uint32 format;
  int access;
  int w;
  int h;
...
#if 1
  size_t pitch;
  auto pixels = makeTestPixels(msg_width, msg_height, chan, pitch, 10, 0, 30);
...
void ViewImage::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
  // TODO(lucasw) does the msg need to persist in memory?
  // msg_ = msg;
